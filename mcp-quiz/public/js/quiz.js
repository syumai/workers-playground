document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.category-btn').forEach(button => {
        button.addEventListener('click', function() {
            ...
            document.getElementById('category-title').textContent = currentCategory + ' Quiz';
    ...
    async function loadQuestions(category) {
        try {
            const response = await fetch(`${category}_questions.json`);
    ...
    function showResults() {
        ...
        for (let i = 0; i < questions.length; i++) {
            ...
            if (isCorrect) {
                reviewAnswer.textContent = `正解: ${question.options[question.correctAnswer]}`;
            } else {
                reviewAnswer.textContent = `あなたの回答: ${question.options[userAnswer]} / 正解: ${question.options[question.correctAnswer]}`;
            }
